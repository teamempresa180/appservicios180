...
void main() {
  group('MockProviderDashboardRepository', () {
    ...
    test('getQuotes returns real Quote entities, not maps', () {
    ...
    test('getPayments returns real Payment entities, not maps', () {
      final payments = repository.getPayments();
      expect(payments, isNotEmpty);
      expect(payments, everyElement(isA<Payment>()));
    });

    test('every order references the same provider returned', () {
      final providerId = repository.getProvider().id;
      expect(
        repository.getOrders().every((o) => o.providerId == providerId),
        isTrue,
      );
    });
    ...
    test('is independent from every other feature mock data', () {
      expect(
        repository.getProvider().id.value.startsWith('provider-dashboard-'),
        isTrue,
      );
    });
  });
